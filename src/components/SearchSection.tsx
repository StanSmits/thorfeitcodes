...
const SearchSection: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const normalizedSearchTerm = searchTerm.toLowerCase().replace(/\s+/g, ''); // Remove spaces from search term
    const filteredSuggestions = factCodes.filter(code => {
      const normalizedCode = code.code.toLowerCase().replace(/\s+/g, ''); // Remove spaces from code
      return (
        normalizedCode.includes(normalizedSearchTerm) || // Match Feitcode without spaces
        code.description.toLowerCase().includes(searchTerm.toLowerCase()) // Match description
      );
    }).slice(0, 8);
